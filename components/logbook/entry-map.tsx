...
export function EntryMap({ lat: latitude, lon: longitude, landingLat: landingLatitude = null, landingLon: landingLongitude = null, label = "Flying site", sites = [], onPick, onPickLanding }: {
  ...
  useEffect(() => { onPickRef.current = placement === "landing" && onPickLanding ? onPickLanding : onPick; }, [onPick, onPickLanding, placement]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (!instance) return;
    function updateMarker(ref: typeof marker, y: number | null, x: number | null, color: string, name: string) {
      if (y == null || x == null) { ref.current?.remove(); ref.current = null; return; }
      if (!ref.current) {
        ref.current = new maplibregl.Marker({ color }).setLngLat([x, y]).addTo(instance!);
    ...
    updateMarker(marker, lat, lon, "#0099ff", "Flying site pin (blue)");
    updateMarker(landingMarker, landingLat, landingLon, "#ea580c", "Landing pin (orange)");
  ...
  return <div className="overflow-hidden rounded-xl border border-gray-200">
    ...
    <div className="flex items-center justify-between gap-3 bg-gray-50 px-3 py-2 text-xs text-gray-600">
      <span>{onPick ? `Click the map to set the ${placement === "landing" ? "landing location" : "flying site"}` : `${label} · approximate site location`}</span>
