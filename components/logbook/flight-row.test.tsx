...
it.each([
  ...
  ["friends", "users"],
  ["private", "lock"],
] as const)("uses a round %s scope icon for compact visibility", (scope, icon) => {
  ...
  expect(visibility.querySelectorAll("svg")).toHaveLength(1);
  expect(visibility.querySelector(`.lucide-${icon}`)).toBeInTheDocument();
});

it("shows one overlapping-paragliders badge for companion matches", () => {
  render(<FlightRow flight={flight} compact friendFlightsFound />);
  const companions = screen.getByLabelText("You flew together");
  expect(companions).toHaveClass("bg-brand-blue");
  expect(companions.querySelectorAll("svg")).toHaveLength(1);
  const companionColumn = companions.parentElement;
  expect(companionColumn).toHaveClass("w-full", "justify-center");
...
it("reserves the companion column for flights without a match", () => {
  ...
  expect(companionColumn).toHaveClass("w-full");
