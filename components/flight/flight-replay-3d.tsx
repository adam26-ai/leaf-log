...
type ReplayData = ReplayResponse;
...
function diagnosticLineSegments(tracks: MultiColorPathDatum[]): DiagnosticLineSegment[] {
  ...
  for (const track of tracks) {
    for (let index = 1; index < track.path.length; index++) {
      segments.push({
        sourcePrevious: track.path[index - 2] ?? track.path[index - 1],
        source: track.path[index - 1],
        target: track.path[index],
        targetNext: track.path[index + 1] ?? track.path[index],
...
export const FlightReplay3D = forwardRef<FlightReplay3DHandle, FlightReplay3DProps>(
  ...
  function tracksFor(d: ReplayData): TimedTrackDatum[] {
  ...
  useEffect(() => {
    ...
    if (map) {
      removeShadow(map);
      if (cameraModeRef.current !== "fixed") centerOnGlider(time, cameraModeRef.current === "chase");
      syncShadow();
      renderLayers(time);
      map.triggerRepaint();
    }
  ...
  function displayedTrackAt(track: TimedTrackDatum, t: number): MultiColorPathDatum {
    ...
    const count = Math.min(track.path.length, low);
    const cached = displayedTrackCacheRef.current.get(track);
    if (cached?.count === count) return cached.value;
    const value = {
      path: track.path.slice(0, count),
      colors: track.colors.slice(0, count),
    };
    displayedTrackCacheRef.current.set(track, { count, value });
    return value;
  }
  ...
  function shadowGeoJson(d: ReplayData, t: number): GeoJsonData {
    return {
      ...
      features: tracksFor(d).map((track) => displayedTrackAt(track, t)).filter((track) => track.path.length >= 2).map((track) => ({
        type: "Feature", properties: {}, geometry: { type: "LineString", coordinates: track.path.map((s) => [s[0], s[1]]) },
      })),
    } as GeoJsonData;
  }
  ...
  function syncShadow() {
    ...
    if (!showShadowRef.current || !d || d.samples.length < 2 || (timeRef.current < 0 && trackDisplayRef.current === "elapsed")) {
      removeShadow(map);
      return;
    }
  ...
  function renderLayers(t: number) {
    ...
    const photoIcons: PhotoIcon[] = photosRef.current.filter(isPinned).flatMap((ph) => {
    ...
    if (showShadowRef.current && state === "Flying") {
      type CurtainSample = {
        time: number;
      ...
      for (let index = 0; index < sampleCount; index++) {
        ...
        if (replayStateAt(d, sampleTime) !== "Flying") continue;
    ...
    poleLayers.push(
      new TextLayer<LabelDatum>({
        ...
        data: [
          {
            text: state === "Flying" ? `${formatAltitude(Math.round(displayAlt), unitsRef.current)} ${isAgl ? "AGL" : "MSL"}` : state,
            position: anchorPos,
          },
        ],
        getText: (l) => l.text,
    ...
    if (diagnosticMode === "path2") {
      ...
      primaryTrackLayers = [
        new PerspectiveOutlinedLineLayer<DiagnosticLineSegment>({
          ...
          data: lineSegments,
          getSourcePreviousPosition: (segment) => linePosition(segment.sourcePrevious),
          getSourcePosition: (segment) => linePosition(segment.source),
          getTargetPosition: (segment) => linePosition(segment.target),
          getTargetNextPosition: (segment) => linePosition(segment.targetNext),
          getColor: (segment) => segment.color,
          getWidth: PRIMARY_TRACK_WIDTH_PX,
          widthUnits: "pixels",
          widthMinPixels: PRIMARY_TRACK_WIDTH_PX,
      ...
      layers: [
        ...
        new ScreenSpaceIconLayer<PhotoIcon>({
          ...
          onHover: (info) => {
            ...
            if (o) {
              setHoverPhoto({ x: info.x, y: info.y, id: o.id, flightId: o.flightId, name: o.name, primary: o.primary });
            } else {
              setHoverPhoto(null);
            }
          },
  ...
  useEffect(() => {
    ...
    const map = new maplibregl.Map({
      ...
      center: [dataRef.current!.samples[0][0], dataRef.current!.samples[0][1]],
      zoom: 12,
      pitch: 62,
      bearing: -20,
      maxPitch: 85,
    ...
    <Card className="overflow-hidden">
      <div className="relative">
        ...
        {hoverPhoto && (
          ...
          <img
            src={photoUrl(hoverPhoto.flightId, hoverPhoto.id, "thumb")}
            alt={hoverPhoto.name}
