...
it("does not fabricate altitude trophies when the original file has no altitude", () => {
  ...
  expect(altitudeMeasurements(parsed, deriveMetrics(parsed))).toEqual({ metricsVersion: 1, launchAltM: null, maxAltM: null });
});
it("uses the alternate altitude source at launch when available", () => {
  const parsed = parseIgc(makeRealisticFlight().igc);
  const metrics = deriveMetrics(parsed)!;
