...
export async function replayArtifactForFlight(flight: Flight): Promise<ReplayArtifact | null> {
  ...
  const work = (async () => {
    ...
    const artifact = readReplayArtifact(cached?.replay, flight.igcSha256, flight.parserVersion);
    ...
    const result = buildReplayArtifact(parsed, metrics, flight.igcSha256, flight.parserVersion);
