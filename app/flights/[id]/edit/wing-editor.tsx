...
export function FlightWingEditor({ flightId, glider, gliders }: {
  ...
  const [savedValue, setSavedValue] = useState(glider);
  const submittedValue = useRef(glider);
  const [state, action, pending] = useActionState(updateFlightWing.bind(null, flightId), {} as NotesState);
  useEffect(() => {
    if (state.ok) {
      const saved = submittedValue.current.trim();
      setValue(saved);
      setSavedValue(saved);
    }
  }, [state]);
  ...
  return (
    <form action={action} onSubmit={() => { submittedValue.current = value; }} className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-3">
        <Button type="submit" variant={value === savedValue ? "outline" : "primary"} disabled={pending || !hydrated || value === savedValue}>{pending ? "Saving…" : "Save wing"}</Button>
